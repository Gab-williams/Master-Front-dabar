...
export default function BlogDetails() {
    ...
    const intervalId = setInterval(() => {
        if(localinfo){
          ...
          let ans =  story.find((item)=>item.heading?.toLowerCase() == fieldsdata.heading?.toLowerCase())
                  ...
                  <div className="col-xl-8 col-lg-7">
                    <div className="blog-details-wrap">
                      ...
                      <div className="blog-prev-next-posts">
                                                <div className="row">
                                                    {random.length > 0? random.map((item, i)=>{
                                                        return  <div className="col-xl-6 col-lg-8 col-md-6" key={i}>
                                                        <div className="pn-post-item">
                                                            <div className="thumb">
                                                                <Link href={`/blog/${item.id}`}><img src={item.thumbnail} alt="img" /></Link>
                                                            </div>
                                                            <div className="content">
                                                                {i == 0?<span>Prev Post</span>:<span>Next Post</span>}
                                                                
                                                                <h5 className="title tgcommon__hover"><Link href={`/blog/${item.id}`}>{item.heading.length >70?item.heading.substr(0, 20)+"..":item.heading}</Link></h5>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    }):""}
